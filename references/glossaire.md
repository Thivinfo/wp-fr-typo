...
> **603 termes** — Dernière mise à jour : **10/06/2026**.
> Maintenu par l'équipe Polyglots WordPress Francophone.

## Règles d'usage

- **Toujours vouvoyer** : « vous », jamais « tu ».
- **plugin** → **extension** (JAMAIS plugin, plug-in, greffon).
- **post** (générique) → **publication** (depuis WP 5.0).
- **WordPress** : W et P majuscules, toujours.
- Noms de thèmes/extensions : ne pas traduire.
- En cas de doute : Slack #traductions ou glossaire en ligne.
...
| `backorder` | en réapprovisionnement | noun | la commande/l’expédition est différée car le produit est en cours de réapprovisionnement |
| `binary` | binaire | noun |  |
| `block Patterns` | compositions de blocs | noun |  |
| `block theme` | thème basé sur des blocs | expression | Si manque de place : thème de blocs |
| `blog` | blog | noun |  |
| `bookmark` | marque-page | noun |  |
...
| `Bounce Rate` | taux de rebond | expression |  |
| `Brand Awareness` | notoriété de la marque | expression |  |
| `branding` | marque | noun |  |
...
| `chat` | discussion | noun | ou discussion instantanée. [np] |
| `chatbot` | chatbot | noun | Terme technique Peut être remplacé par robot conversationnel si l’interface le permet |
| `chatbot` | robot conversationnel | noun | Utiliser chatbot si l’interface ne le permet pas |
| `Cheatin’ uh?` | Une fausse manipulation&nbsp;? | noun |  |
| `checkbox` | case à cocher | noun |  |
| `checklist` | liste à cocher | noun |  |
...
| `click` | cliquer | verb |  |
| `click` | clic | noun |  |
| `client` | client | noun | Client/cliente en e-commerce |
| `clipboard` | presse-papier | noun |  |
| `cloud` | cloud | noun |  |
...
| `credentials` | informations de connexion | noun | ou infos de connexion |
| `crop` | recadrer | verb |  |
...
| `Customer Relationship Management` | gestion de la relation client | expression | Abréviation en anglais : CRM |
...
| `data controller` | responsable du traitement | noun | Dans le contexte du RGPD (GDPR). Note : le terme « processor » est quant à lui traduit par « sous-traitant ». |
| `date field` | champ de date | noun |  |
| `date picker` | sélecteur de date | noun |  |
| `date range` | plage de dates | noun |  |
| `deactivate` | désactiver | verb |  |
| `deactivating` | désactivation | noun |  |
| `debug` | débogage | noun |  |
| `debug` | déboguer | verb |  |
...
| `email marketing` | marketing par e-mail | expression |  |
| `emails` | e-mails | noun |  |
...
| `Full size` | Taille originale | noun |  |
...
| `header` | entête | noun | Nom masculin |
| `Heading` | Titre | noun |  |
| `headline` | titre | noun |  |
| `help` | aide | noun |  |
...
| `issuer` | émetteur/émettrice | noun | Généralement associé aux personnes qui rédigent une issue ou un rapport de bug |
...
| `lead` | prospect | noun | Lorsque utilisé dans le commerce en ligne. S‘il s'agit du sens lié au « leadership », on utilise dirigeant/diriger. |
| `lead generation` | génération de prospects | expression |  |
| `legacy` | ancien | noun | ancien/ancienne |
...
| `Location` | Emplacement | noun | Emplacement d’un élément (interface) [np] |
| `Location` | Localisation | noun | Localisation géographique (lieu) [np] |
...
| `loopback` | boucle de requête | noun |  |
...
| `market research` | étude de marché | expression |  |
| `marketing mix` | mix marketing | expression |  |
...
| `meta` | métadonnée | noun |  |
| `meta` | métadonnées | noun |  |
| `meta` | méta | noun |  |
...
| `Nickname` | Pseudonyme | noun |  |
| `Nightly` | Version du jour | noun |  |
| `No` | Aucun | noun | P. ex. No %s found = Aucun %s trouvé [np] |
| `nonce` | nonce | noun |  |
...
| `Pay-per Click` | Paiement par clic | expression | Abréviation en anglais : PPC |
...
| `permissions` | droits | noun | Droits d’accès, droits en écriture / de modification / de suppression [np] |
...
| `plan` | offre | noun | Marketing [np] Dans le contexte de learn.wordpress : plan |
...
| `request` | requête | noun | dans les contextes techniques |
...
| `Return on Investment` | retour sur investissement | expression | Abréviation en anglais : ROI |
...
| `Search Engine Optimization` | optimisation pour les moteurs de recherche | expression | Abréviation en anglais : SEO |
...
| `Social Media Marketing` | Marketing des médias sociaux | expression | Abréviation en anglais : SMM |
| `socket` | connecteur | noun |  |
...
| `store` | boutique | noun | e-commerce |
| `string` | chaine | noun |  |
...
| `support` | prendre en charge | verb | P. ex. : votre site prend en charge PHP 8.0 [np] |
| `survey` | enquête | noun |  |
...
| `tagline` | slogan | noun |  |
| `target audience` | public cible | expression |  |
...
| `text domain` | text domain | noun | ou « domaine de traduction » éventuellement |
...
| `translator` | traducteur/traductrice | noun |  |
...
| `Unique Selling Proposition` | proposition de vente unique | expression | Abréviation en anglais : USP |
...
| `widget` | widget | noun |  |
| `woocommerce` | WooCommerce | noun | C'est une marque |
